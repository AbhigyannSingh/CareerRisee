...
export const AppliedJobs = () => {
    ...
    useEffect(() => {
        // Fetch applied jobs data and dispatch it to Redux store
        const fetchAppliedJobs = async () => {
            // Replace with your actual fetching logic (e.g., API call)
            const response = await fetch('/api/appliedJobs'); // Example API endpoint
    ...
    return (
        ...
        >
            ...
            <Table className="my-2 max-sm:overflow-x-auto max-sm:w-[640px]">
                <TableCaption className="max-sm:text-left">
                        ...
                        appliedJobs.map((application, index) => (
                            <TableRow key={index}>
                                ...
                                <TableCell className="text-center">
                                    <Badge
                                        variant="outline"
                                        className={`px-4 text-base text-slate-600 cursor-default ${
                                            application?.status === 'accepted' && 'bg-green-200'

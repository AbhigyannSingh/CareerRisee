...
const AdminJobs = () => {
    ...
    const adminJobs = useSelector((store) => store.job.adminJobs || []); // Corrected to access `adminJobs` directly
    ...
    useEffect(() => {
        // Update `filterJobs` whenever `searchText` or `adminJobs` changes
        setFilterJobs(
            adminJobs.filter(
                (job) =>
    ...
    return (
        <div className='max-w-7xl mx-auto my-10 max-md:pl-2'>
            ...
            <div className='flex items-center gap-4 my-10 max-sm:flex-col max-sm:w-[95%]'>
                ...
                />
                <Button onClick={() => navigate('/admin/jobs/create')} className='p-6 px-10 bg-cyan-500 hover:bg-cyan-700 max-sm:self-end'>
                    Post New Job
                </Button>
            </div>
            <div>
                            ...
                            filterJobs.map((job, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell className='flex items-center gap-4 justify-center'>
                                        <Button variant='outline' onClick={() => navigate(`/admin/jobs/${job?._id}`)}>
                                            Details
                                        </Button>
                                    </TableCell>
                                </TableRow>
